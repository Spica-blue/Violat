...
import styles from '../styles/TradeLog.module.css';
...
export default function TradeLog({ reloadLog }) {
    ...
    useEffect(() => {
        if (accountNum) {
            const fetchData = async () => {
                try {
                    const detailResponse = await axios.post('http://127.0.0.1:8000/portfolio/inDeTradeLog/', { account_num: accountNum });
                    setLogData(detailResponse.data);
                    console.log("TradeLog updated:", detailResponse.data);
                } catch (error) {
    ...
    }, [accountNum, reloadLog]); // reloadLog 추가!
