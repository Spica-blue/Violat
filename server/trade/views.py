...
import asyncio
from .api import get_filter_data, fetch_stock_info, get_stock_code_by_name, connect, start_websocket_server, load_realtime_stock_data
...
def start_websocket_server_in_thread():
    ...
    if not websocket_server_running:
        try:
            ...
            print(f"Error in websocket server: {str(e)}", flush=True)
...
def connect_to_realtime_data_in_thread():
    try:
        ...
        print(f"Error in connecting to real-time data: {str(e)}", flush=True)
